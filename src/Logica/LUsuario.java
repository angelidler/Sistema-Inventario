...
public class LUsuario {
    ...
    public DefaultTableModel MostUsuario(DUsuarios MisUsuario){
        ...
        try {
            ...
            miModulo=new DefaultTableModel(null,Titulos);
                CallableStatement cst=c.prepareCall("{ call sp_buscar_us (?)}");
    ...
    public  String InsartarUs(DUsuarios misUsuarios){
      String msj=null;
        try {
            CallableStatement cst=c.prepareCall("{call sp_insertar_us(?,?,?,?)}");
     ...
     public  String EditarUs(DUsuarios misUsuarios){
      String msj=null;
        try {
            CallableStatement cst=c.prepareCall("{call sp_editar_us (?,?,?,?,?)}");
     ...
     public  String EliminarUs(DUsuarios misUsuarios){
      String msj=null;
        try {
            CallableStatement cst=c.prepareCall("{call sp_eliminar_us (?)}");
            cst.setInt(1, misUsuarios.getIdUsuario());
            cst.executeUpdate();
            msj="si";
        } catch (Exception e) {
